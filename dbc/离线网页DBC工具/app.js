(function(){
  ...
  Project.prototype.toDBC = function(){
    ...
    lines.push('');
    lines.push('NS_:'); for(var i=0;i<NS.length;i++) lines.push('  '+NS[i]);
    lines.push(''); lines.push('BS_:'); lines.push('');
    lines.push('BU_: ' + this.nodes.join(' '));
    lines.push('');
    for(var j=0;j<this.messages.length;j++){ lines.push(this.messages[j].toDBC()); lines.push(''); }

    // 值表
    for(var name in this.valueTables){
      if(!this.valueTables.hasOwnProperty(name)) continue;
      var items=this.valueTables[name]||[], parts=[];
      for(var k=0;k<items.length;k++){ parts.push(items[k][0]+' "'+escQuote(items[k][1])+'"'); }
      lines.push('VAL_TABLE_ '+name+' '+parts.join(' ')+' ;');
    }
    // 备注
    for(j=0;j<this.messages.length;j++){
      var m=this.messages[j];
      if(m.comment) lines.push('CM_ BO_ '+m.canId+' "'+escQuote(m.comment)+'";');
      for(k=0;k<m.signals.length;k++){ var s=m.signals[k]; if(s.comment) lines.push('CM_ SG_ '+m.canId+' '+s.name+' "'+escQuote(s.comment)+'";'); }
    }
    // VAL_ 绑定
    for(j=0;j<this.messages.length;j++){
      m=this.messages[j];
      for(k=0;k<m.signals.length;k++){
        ...
        if(s.valTable && this.valueTables[s.valTable]){
          ...
          for(var t=0;t<items.length;t++){ parts.push(items[t][0]+' "'+escQuote(items[t][1])+'"'); }
  ...
  function renderMsgList(){
    ...
    for(var i=0;i<proj.messages.length;i++){
      (function(idx){
        ...
        a.type = 'button';
        a.className = 'list-group-item list-group-item-action' + (idx===selectedMsgIndex?' active':'');
    ...
    for(var i=0;i<m.signals.length;i++){
      (function(i){
        ...
        tr.innerHTML = [
          ...
          '<td><input class="form-control form-control-sm" data-k="valTable" value="'+(s.valTable||'')+'" placeholder="表名"></td>',
  ...
  function renderVtList(){
    ...
    document.getElementById('vtList').textContent = names.length? names.join(', ') : '(无)';
  }
  document.getElementById('vtAdd').addEventListener('click', function(){
  ...
  els.fileImport.addEventListener('change', function(){
    ...
    (async function(){
      ...
      renderNodes(); renderMsgList(); openMsg(); refreshPreview();
